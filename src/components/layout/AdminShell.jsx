...
export function AdminShell() {
  ...
  const notices = [gamesData, platformsData, usersData, ordersData];
  ...
  return (
    <AdminLayout
      ...
      action={['games', 'users', 'platforms'].includes(section)
        ? {
    ...
    >
      ...
      {modal === 'user' && 
        <Modal title="Nuevo cliente" onClose={() => setModal(null)}>
          <UserForm onSubmit={(values, formik) => submit(usersData.saveUser, values, formik)} onCancel={() => setModal(null)} />
        </Modal>}
      {modal === 'platform' && 
